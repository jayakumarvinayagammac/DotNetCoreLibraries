    ...
    public static class Endpoints
    {
        public static void MapArtistEndpoints(this IEndpointRouteBuilder endpoints)
        {
            endpoints.MapGet("/artists", async (IArtistService service) =>
                Results.Ok(await service.GetAllAsync())).WithName("GetAllArtists").WithTags("Artists");
            ...
            }).WithName("GetArtistById").WithTags("Artists");

            endpoints.MapPost("/artists", async (Artist artist, IArtistService service) =>
                Results.Created($"/artists/{artist.ArtistId}", await service.CreateAsync(artist))).WithName("CreateArtist").WithTags("Artists");
            ...
            }).WithName("UpdateArtist").WithTags("Artists");

            endpoints.MapDelete("/artists/{id:int}", async (int id, IArtistService service) =>
                await service.DeleteAsync(id) ? Results.NoContent() : Results.NotFound()).WithName("DeleteArtist").WithTags("Artists");
        }
    }
}
